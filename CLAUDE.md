...
- **整合待辦（正式進 index.html 前）**：① 浮水印切字位置／浮雕深淺待創辦人用 index-rain-preview 左下調軸定數值、再寫死移面板 ② 服務區第三塊內容待定（數位轉型顧問／自有產品，見 [[project_nestdigital_service_taxonomy]]）③ 堆疊卡內容（開發/成長/顧問）對應正確服務分類待全局對齊 ④ 進 5 層 CSS（pages/index.css + index.js）⑤ 效能（Canvas2D 程式碼雨 + GSAP 堆疊）。
