...
### 數位行銷手風琴「展開態」背景設計進度（2026-06-18 17:39，進行中）

對應任務 11b。手風琴展開後（`.body`）每張服務卡的**白色系背景圖**設計。沙盒＝`demo-liquid/service-accordion-expand-art.html`（依檔內 `open(n)` 切換看哪張卡、上方工具列切背景版本）；**全部為預覽、未進正式 index.html**。圖由 codex `image_gen` 生成、存 `_imgtest/`。

**定型原則（創辦人多輪拍板）：**
- **每張卡「各自不同風格」，不是同一基底 swap 母題**（曾想拿社群紙雕當模板套四張被否決＝偷懶/框架化）。共用的只有「精神」：白亮/淺底（跳出整站深黑、不死冷）＋該服務「真實可辨識元素」＋藝術感＋不單調也不花俏（藝術＋元素結合、不要彩帶 confetti 廉價裝飾）＋不工程師味（受眾一般大眾）。
- **背景先當完整作品設計、不為文字讓位**；知道文字大概落「左上往下」即可。背景定稿後才處理內容怎麼擺、需要才加遮罩。多數卡定稿後會把主元素往右收、左上清乾淨給文字。
- **人物一律台灣人**（AI 生圖預設西方臉，必須明確指定台灣/東亞）。

**四張卡進度：**
- **社群經營＝已定** `_imgtest/social-mix-a2.png`：暖白紙雕浮雕藝術底 ＋ 彩色 3D 平台 icon（LINE/IG/FB/Threads/TikTok）＋ 紙雕社群成長母題（上升箭頭/讚/愛心/泡泡）有上升動勢。
- **部落格代營運＝已定** `_imgtest/blog4-b.png`：暖金電影感 3D，一大群台灣太空人工人「背對鏡頭、無臉」扛 SEO/GEO/#1/關鍵字內容、擁擠衝向發光「權威燈塔」；大元素集中右側、左上留白。
- **廣告投放＝已定** `_imgtest/ads2-a.png`：暖金 3D「命中靶心」，廣告卡正中紅心、ROAS/CTR/CPA/轉換迸發；大靶心在右、左上留白。
- **AI 形象網紅＝未定（下一步要先處理）**：概念已鎖＝**台灣女性 KOL 從手機 IG 社群首頁「破框走出」**（站在手機前、前後景深清楚，非卡在螢幕裡；手自然）。候選四種姿勢 `_imgtest/ai-v2-a.png`（側身走出）/`ai-v2-b.png`（跨步而出）/`ai-v2-c.png`（大步動感）/`ai-v2-d.png`（倚框而坐）**待創辦人挑**；挑定後把人物往右收、左上留白再鎖定。

**待辦（接續順序）：** ① AI 形象網紅挑版＋微調定稿 → ② 四卡背景齊 → ③ 設計展開態版面/文字排版（標題＋說明＋方案/價格＋CTA，必要處加遮罩保可讀）→ ④ 整合進正式 index.html（接月面碑刻收合態）。
