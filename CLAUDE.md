...
### 設計方向（2026-05-06 補入，避免後續 session 誤判）

**品牌核心 = 跳躍思維 / 非常規（unconventional / leap-thinking）**

源自 h1 主標「用非常規的跳躍思維解決習以為常卻永遠解決不了的問題」、副標「敢用不同方法的團隊」。整個品牌訴求是「跳出框架、主動破局」。

**早期曾用「天文觀測站 / 觀測儀」框架已棄用**——觀測儀的「被動觀察 / 科學測量」氣質與品牌核心相違。但 codebase 中有大量觀測儀殘留尚未清理，包括：

- Three.js 黑洞星場（`#blackhole` + `initStarfield()`）
- CSS 星空 `#css-starfield`（`base.css`）
- `.nav__brand-star` 暖白星點 + 望遠鏡光暈
- 各種「螺旋臂 / 軌道 / 星辰」相關 token / 註解 / class 名稱

**處理原則：** 2026-05-06 已完成「**第一階段：純用詞 / 註解清理**」（零視覺影響）。第二階段（視覺實作清理）待創辦人逐項拍板，因為每項都會影響當前已確認的設計。

**未來碰到「視覺要不要保留」的決策，預設答案是「往跳躍思維演化」**——跳躍／非常規／斷裂／錯位／角度／弧線軌跡 = 對齊；觀測／儀器／刻度／規律／對稱／中央構圖 = 違和需重設計。
