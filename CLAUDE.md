...
- 部落格**顯示名定為「不正常觀點」**（接「不正常人類軟體開發」起源＋「觀點」一看就懂＋順便篩客戶），**網址路徑維持 `/blog`**（中文當顯示名、英文當路徑；nav label 對 SEO 無影響、中文網址會被編碼成亂碼故不用）。**時機**：部落格上線前不在 nav 放連結（避免斷鏈），先規劃版位。
- nav 目標結構：**關於 · 服務 · 不正常觀點 · 聯絡我們**（拿掉「消息」news.html、由部落格吸收；「服務」維持錨點跳首頁 #services-section，與服務不開頁一致）。**待確認**：① 拿掉「消息」② 「關於」維持獨立頁（需補文案 task #9）或收進首頁成一區塊。
