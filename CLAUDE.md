...
| Hero ANOMALY 背景大字 | 2026-04-19 規劃為 Variable Font 軸動文字（Fraunces 三軸 opsz/wght/SOFT 由 GSAP ScrollTrigger 驅動） | 2026-05-06 創辦人要求移除（commit d7c4913）。HTML `.hero__bg-text` div、CSS 桌機規則與手機 media query、JS `initVariableFontAxis()` 中針對 hero 的 GSAP scrollTrigger 區塊全部刪除。**保留**：`@property --vf-opsz/--vf-wght/--vf-soft` 與 `initVariableFontAxis()` 函式（仍供 `.manifesto__bg-text` UNCONVENTIONAL 使用）。 |
